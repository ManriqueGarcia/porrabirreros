...
import { loadHistorical, saveBetF1, saveQuestionsF1, skipNextRemoteSave } from "../api.js";
    ...
    <div className="card card-racing p-4 md:p-5 min-w-0">
      ...
      {race && !raceOff && owner===user && authorDeadline && now<authorDeadline && !(db.questionsStatus?.[race.key]?.locked) && (
        <div className="mb-3 space-y-2 bg-neutral-900 border border-white/10 rounded p-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">{[0,1,2].map(i=>(<input key={i} className="select border rounded px-3 py-2 w-full" placeholder={"Pregunta "+(i+1)} value={(db.questions?.[race.key]?.[i]||"")} onChange={e=>{const val=e.target.value; setDb(prev=>{const curr=prev.questions?.[race.key]||["","",""]; const next=[...curr]; next[i]=val; return {...prev, questions:{...(prev.questions||{}), [race.key]: next}};});}}/>))}</div>
          <div className="flex gap-2">{!db.questionsStatus?.[race.key]?.published ? (<button className="px-3 py-2 rounded bg-emerald-600 text-white" onClick={async()=>{ const list=(db.questions?.[race.key]||["","",""]); if(list.some(q=>!q||!q.trim())) return toast.error("Rellena las 3 preguntas"); try{ await saveQuestionsF1(race.key,list,true); skipNextRemoteSave(); setDb(prev=>({...prev, questionsStatus:{...(prev.questionsStatus||{}), [race.key]:{published:true, author:user, publishedAt:new Date().toISOString()}}})); toast.success("Publicado"); }catch(err){ console.error("Error publicando preguntas:",err); toast.error("No se pudo publicar. Inténtalo de nuevo."); } }}>Publicar</button>):(<button className="px-3 py-2 rounded bg-amber-600 text-white" onClick={async()=>{ const list=(db.questions?.[race.key]||["","",""]); if(list.some(q=>!q||!q.trim())) return toast.error("Rellena las 3 preguntas"); try{ await saveQuestionsF1(race.key,list,false); skipNextRemoteSave(); setDb(prev=>({...prev, questionsStatus:{...(prev.questionsStatus||{}), [race.key]:{...(prev.questionsStatus?.[race.key]||{}), updatedAt:new Date().toISOString()}}})); toast.success("Actualizado"); }catch(err){ console.error("Error actualizando preguntas:",err); toast.error("No se pudo actualizar. Inténtalo de nuevo."); } }}>Actualizar</button>)}</div>
        </div>
      )}
