...
import { FUTBOL_CAT_PENALTY_EXCLUDED_USERS } from "./futbol-cat-excluded.mjs";
...
export function scoreForRace(db, raceKey, name, raceFromCalendar, userCreatedAt) {
  ...
  if (noBet) {
    return {
      ...
      gotPole: false, gotAllPodium: false, gotAllQuestions: false, fullHouse: false,
      submittedAt: null, missed: realResults, late: false, notYetJoined: false,
    };
  }
  if (!realResults) {
    return {
...
export function computeGlobalStandings(db, races, participantsOverride, usersMap) {
  ...
  return participants.map(name => {
    ...
    A.pen - B.pen || A.avgSubmit - B.avgSubmit || A.createdTs - B.createdTs
  );
}
